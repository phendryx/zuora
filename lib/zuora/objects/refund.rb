module Zuora::Objects
  class Refund < Base
    ...
    validates_inclusion_of    :method_type, :in => %w(ACH Cash Check CreditCard Other PayPal WireTransfer DebitCard CreditCardReferenceTransaction), :if => lambda { self.type == "External" }
    validates_length_of       :payment_method_id, :maximum => 60
    validates_datetime_of     :refund_date, :if => lambda { self.type == "External" }
